...
def segmentation_tables(runs_dir: Path, seg_dir: str = "seg") -> tuple[list[str], dict]:
    groups: dict[tuple, list[dict]] = defaultdict(list)
    for p in sorted(runs_dir.glob(f"{seg_dir}/*/results.json")):
        r = json.loads(p.read_text())
        a = r["args"]
...
def main() -> None:
    ...
    seg_dirs = sorted(d.name for d in runs.glob("seg*") if d.is_dir())
    s_lines, s_sum = [], {}
    for d in seg_dirs:
        lines_d, sum_d = segmentation_tables(runs, d)
        s_lines += lines_d
        s_sum["segmentation" if d == "seg" else f"segmentation_{d}"] = sum_d
    md = ["# Result tables (auto-generated by scripts/collect_results.py)", ""] + g_lines + c_lines + s_lines
    Path(args.out_md).parent.mkdir(parents=True, exist_ok=True)
    Path(args.out_md).write_text("\n".join(md))
    Path(args.out_json).parent.mkdir(parents=True, exist_ok=True)
    Path(args.out_json).write_text(json.dumps({"generation": g_sum, "checkpoints": c_sum, **s_sum}, indent=2))
    print("\n".join(md))
